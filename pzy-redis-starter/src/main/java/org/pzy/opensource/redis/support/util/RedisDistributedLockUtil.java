...
public class RedisDistributedLockUtil {
    ...
    private RedisDistributedLockUtil() {
    }

    public static RedisDistributedLockUtil INSTANCE;

    public static RedisDistributedLockUtil newInstance(RedisTemplate<String, Object> redisTemplate) {
        if (null == REDIS_TEMPLATE_VALUE_JSON_STRING) {
            synchronized (RedisDistributedLockUtil.class) {
                if (null == REDIS_TEMPLATE_VALUE_JSON_STRING) {
                    REDIS_TEMPLATE_VALUE_JSON_STRING = redisTemplate;
                    INSTANCE = new RedisDistributedLockUtil();
                }
            }
        }
        return INSTANCE;
     ...
     */
    public static boolean tryLock(String lockKey, String clientId, Long expireSecond, Integer maxTryCount, Long sleepMilliscond) throws InterruptedException {
        boolean hasLock = RedisUtil.setIfAbsent(lockKey, clientId, expireSecond);
        if (!hasLock) {
            int tryCount = 0;
            ...
            while (tryCount < maxTryCount) {
                ...
                hasLock = RedisUtil.setIfAbsent(lockKey, clientId, expireSecond);
                if (hasLock) {
                    return hasLock;
                }
            }
     ...
     * @throws InterruptedException 当休眠被中断时,抛出该异常
     */
    public static boolean tryLock(String lockKey, String clientId, Long expireSecond) throws InterruptedException {
        return tryLock(lockKey, clientId, expireSecond, 3, 300L);
    }
